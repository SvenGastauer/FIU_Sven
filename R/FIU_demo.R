...
FIUSphereDemo <- function(){
  ...
  shiny::shinyApp(
    ...
    ui <- shiny::fluidPage(
      ...
      shiny::sidebarLayout(
        shiny::sidebarPanel("Anderson Analytical Sphere",
      sliderInput('a', 'Radius a [mm]',
                  min=0, max=1000, value=10,
                  step=1, round=0),
      sliderInput("frange", label = h3("Frequency Range"), min = 1,
                  max = 1000, value = c(18, 200)),
      numericInput("r", "Range from transducer [m]:", 10, min = 1, max = 10000000),
      numericInput("c", "Sound speed in surrounding fluid [m/s]:", 1480, min = 1, max = 10000),
      numericInput("h", "Sound speed contrast:", 1.035, min = 0.1, max = 1.15),
      numericInput("g", "Range from transducer:", 1.025, min = 0.1, max = 1.15),
      numericInput("rho", "Density of surrounding fluid:", 1026.8, min = 100, max = 10000),
      br(),
